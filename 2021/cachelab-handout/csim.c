...
{
	mem_addr_t set_index = addr&set_index_mask;
	cache_set_t set = cache[set_index >> b];
	mem_addr_t tag = addr >> (b+s);
	int i;
	int hit = 0;
	int miss = 1;
	int full = 1;
	cache_line_t *MRUline;
	unsigned long long int cmru = 0;
	for(i=0; i < E; i++){
		if(set[i].valid == 1 && set[i].tag == tag){
			...
			mru_counter++;
			hit_count++;
		}else if(set[i].valid == 0){
			full = 0;
			MRUline = &set[i];
		}else{
			if(set[i].mru > cmru && full){
				cmru = set[i].mru;
				MRUline = &set[i];
			}
		}
    ...
    while( fgets(buf, 1000, trace_fp) != NULL) {
         ...
         */
			...
			if(' ' == instruction){
				...
				sscanf(buf+1, "%c", &instruction);
				if(verbosity == 1){
					printf("%c %llx,%u ", instruction, addr, len);
				}
				accessData(addr);
				if('M' == instruction){
					accessData(addr);
